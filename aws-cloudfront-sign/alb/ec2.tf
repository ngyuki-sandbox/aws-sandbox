...
locals {
  ec2_user_data = <<-EOS
    ...
    write_files:
      ...
      - path: /var/www/html/static/style.css
        content: ${jsonencode(file("${path.module}/files/style.css"))}

      - path: /var/www/html/static/script.js
        content: ${jsonencode(file("${path.module}/files/script.js"))}

      - path: /opt/app/secret/rsa_key_pair_id
        content: ${jsonencode(var.key_pair_ids.rsa)}

      - path: /opt/app/secret/rsa_private_key
        content: ${jsonencode(var.private_keys.rsa)}

      - path: /opt/app/secret/ecdsa_key_pair_id
        content: ${jsonencode(var.key_pair_ids.ecdsa)}

      - path: /opt/app/secret/ecdsa_private_key
        content: ${jsonencode(var.private_keys.ecdsa)}
  EOS
}
